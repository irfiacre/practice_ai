    ...
    <section className="px-36 py-10 align-middle max-md:px-5">
      <div className="">
        <div className="space-y-5 text-center">
          ...
          {"  "}
          <span className="text-black-500 text-5xl font-semibold max-md:text-3xl">
            Driver Hub?
          </span>
        ...
        <div className="flex flex-row items-center justify-around gap-10 pt-10 max-md:flex-wrap">
          {cardContent.map((item) => (
            ...
            >
              <div className="bg-icon_background w-14 h-14 rounded-lg rounded-ss-3xl rounded-ee-3xl">
                <Icon
                  icon={item.icon}
                  fontSize={48}
                  className="text-black-500 -mx-4"
                />
              </div>
              <p className="text-black-500 text-3xl font-bold">
                {item.title}
              </p>
              <p className="text-text_light text-sm">{item.description}</p>
            </BaseCard>
          ))}
        </div>
      </div>
